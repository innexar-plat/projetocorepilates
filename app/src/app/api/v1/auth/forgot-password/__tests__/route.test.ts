...
import { checkForgotPasswordRateLimit, checkPublicRateLimit } from '@/lib/rate-limit';
...
jest.mock('@/lib/rate-limit', () => ({
  ...
  checkForgotPasswordRateLimit: jest.fn(),
}));
...
const checkForgotPasswordRateLimitMock = checkForgotPasswordRateLimit as jest.Mock;

describe('POST /api/v1/auth/forgot-password', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    checkPublicRateLimitMock.mockReturnValue(true);
    checkForgotPasswordRateLimitMock.mockReturnValue(true);
  });
  ...
  it('returns 429 when email rate limit is exceeded', async () => {
    checkForgotPasswordRateLimitMock.mockReturnValue(false);
